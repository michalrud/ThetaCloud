...
struct WireProxy
{
	void begin(uint8_t a, uint8_t b) {mock->begin(a, b);}
	void beginTransmission(uint8_t a) {mock->beginTransmission(a);}
	void endTransmission() {mock->endTransmission();}
	void write(uint8_t a) {mock->write(a);}
	void requestFrom(uint8_t address, uint8_t n) {mock->requestFrom(address, n);}
	uint8_t available() {return mock->available();}
	uint8_t read() {return mock->read();}
